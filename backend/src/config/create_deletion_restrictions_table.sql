...
CREATE TABLE public.users (
  ...
  -- 사용 시간 (분 단위)
  total_usage_minutes BIGINT DEFAULT 0,              -- 총 사용 시간 (분)
...
-- 4-1. 사용 시간 추가 (분 단위)
CREATE OR REPLACE FUNCTION add_usage_minutes(delta_minutes BIGINT)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  UPDATE users
  SET total_usage_minutes = COALESCE(total_usage_minutes, 0) + GREATEST(delta_minutes, 0)
  WHERE id = auth.uid();
END;
$$;

GRANT EXECUTE ON FUNCTION add_usage_minutes(BIGINT) TO authenticated;
REVOKE EXECUTE ON FUNCTION add_usage_minutes(BIGINT) FROM anon;
