...
### 🚀 **배포 설정**
- **포트**: 환경변수 PORT (기본 3001)
- **프로세스 관리**: PM2 또는 Docker 활용 가능
- **헬스체크**: `/api/health` 엔드포인트

### 📊 **성능 최적화**
- **연결 풀링**: Supabase 자동 관리
- **에러 캐칭**: 모든 레이어에서 예외 처리
...
### ✨ **대기업 수준 설계**
- **레이어 분리**: Controller → Service → Database
- **의존성 주입**: 서비스간 느슨한 결합
- **에러 처리**: 일관된 에러 응답 구조
- **로깅**: 구조화된 로그 시스템
...
### 🛡️ **안정성**  
- **입력 검증**: 모든 API 입력값 검증
- **인증**: Supabase JWT 기반 보안
- **예외 처리**: 예상 가능한 모든 에러 상황 대응
...
## 🔗 연관 문서
- [`REFACTORING_SUMMARY.md`](./backend/REFACTORING_SUMMARY.md): 리팩토링 상세 보고서
- [`frontend.md`](./README.md): 프론트엔드 구조 가이드
